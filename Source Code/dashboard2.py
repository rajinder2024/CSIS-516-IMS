...
from sales_report_class import SalesReportClass # This is correct if the class exists in sales_report.py
...
class IMS:
    def __init__(self, root):  
        ...
        # Activity Log Button
        btn_activity_log = Button(
            RightMenu, text="Activity Log", 
            image=self.icon_activity, compound=LEFT, anchor="w",
            font=("times new roman", 20), bg="white", bd=3, cursor="hand2", padx=10
        )
        btn_activity_log.pack(side=TOP, fill=X, pady=5)
        ...
        self.activity_text.insert(END, "Loading recent activity...")
    ...
    def employee(self):
        ...
        self.new_obj.set_current_user(self.current_user_id)
        # Check if current_user_id is set before proceeding
        if self.current_user_id is not None:
            self.new_obj.set_current_user(self.current_user_id)
        else:
            print("current_user_id is not set yet")
    ...
    def update_content(self):
        ...
        try: 
            ...
            self.lbl_sales.config(text=f'Total Sales\n [{str(sales_count)}]')
            ############## Get low stock alert
            cur.execute("SELECT pid, name, qty FROM product WHERE CAST(qty AS INTEGER) < 5")
    ...
    '''def update_recent_activity(self):
        """Update the recent activity text widget with comprehensive activity log"""
        try:
            # Get recent activities
            activities = ActivityLogger.get_recent_activities(limit=5)
            
            self.activity_text.config(state=NORMAL)
            self.activity_text.delete(1.0, END)
            
            if activities:
                self.activity_text.insert(END, "Recent Activities:\n", "bold")
                for activity in activities:
                    self.activity_text.insert(END, f"• {activity}\n")
            else:
                self.activity_text.insert(END, "No recent activities found\n")
            
            self.activity_text.tag_config("bold", font=("Helvetica", 12, "bold"))
            self.activity_text.config(state=DISABLED)
            
        except Exception as ex:
            self.activity_text.config(state=NORMAL)
            self.activity_text.delete(1.0, END)
            self.activity_text.insert(END, f"Error loading activity: {str(ex)}")
            self.activity_text.config(state=DISABLED)'''
