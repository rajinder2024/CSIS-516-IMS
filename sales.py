...
class salesClass:
    def __init__(self, root):  
        ...
        lbl_invoice=Label(self.root,text="Invoice No.",font=("times new romans",15),bg="white").place(x=50,y=100)
        ...
        btn_clear=Button(self.root,text="Clear",command=self.clear,font=("times new roman",15,"bold"),bg="gray",fg="white",cursor="hand2").place(x=490,y=100,width=120,height=28)

        ###### Bill List
        sales_Frame=Frame(self.root,bd=3,relief=RIDGE)
        sales_Frame.place(x=50,y=150,height=330,width=200)

        scrolly=Scrollbar(sales_Frame,orient=VERTICAL)
        self.Sales_List=Listbox(sales_Frame,font=("goudy ild style",15),bg="white",yscrollcommand=scrolly.set)
        ...
        self.bill_photo = self.bill_photo.resize((450,300),Image.Resampling.LANCZOS) # resize image 
        self.bill_photo = ImageTk.PhotoImage(self.bill_photo) 

        lbl_iamge=Label(self.root,image=self.bill_photo,bd=0).place(x=700,y=110)
        self.show()
